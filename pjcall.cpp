...
{
    s_account *callAcc;
    callAcc = (s_account*)user_data;
    PJ_UNUSED_ARG(media_port);
    qDebug() << " Hey file is finished do something!";
    ...
    try {
        ...
        if(!callAcc->FilePlayPath.isEmpty() && ci.remOfferer){              // if a anuncement is configured and call is incoming create a player
            ...
            if (status != PJ_SUCCESS){
                ...
                PJSUA2_CHECK_EXPR( pjsua_conf_connect(pjsua_player_get_conf_port(callAcc->player_id), callId));
