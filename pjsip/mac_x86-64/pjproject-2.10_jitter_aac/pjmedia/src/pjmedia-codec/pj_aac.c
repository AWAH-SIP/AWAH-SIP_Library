...
#include <pjmedia-codec/pj_aac.h>
#include <FDK_audio.h>
...
static pj_status_t aac_encoder_open(HANDLE_AACENCODER *hAacEnc, const pjmedia_codec_param *attr, const struct aac_encoder_params *aac_params)
{
    ...
    aacEncoder_SetParam(*hAacEnc, AACENC_SIGNALING_MODE, 2);
    ...
    aacEncoder_SetParam(*hAacEnc, AACENC_BITRATEMODE, 0);
    ...
    struct aac_encoder_params *aac_fmtp)
{
    ...
    aac_fmtp->aot = AOT_AAC_LC;
    /* Assume only mono/stereo for now */
    aac_fmtp->channel_mode = MODE_2;
...
PJ_DEF(pj_status_t)
pjmedia_codec_aac_init(pjmedia_endpt *endpt)
{
                                  ...
                                  const pjmedia_codec_info *info)
{
    ...
    /* Check clock-rate */
    if (info->clock_rate == 8000 || info->clock_rate == 12000 || info->clock_rate == 16000 || info->clock_rate == 24000 || info->clock_rate == 48000)
    {
        return PJ_SUCCESS;
    }
                                    ...
                                    const pjmedia_codec_info *id, pjmedia_codec_param *attr)
{
    ...
    attr->info.avg_bps = 192000;
    attr->info.max_bps = 224000;
    ...
    default_aac_params.aot = AOT_AAC_LC;
    /* Assume only mono/stereo for now */
                                  ...
                                  pjmedia_codec_param *attr)
{
    ...
    aacDecoder_SetParam(aac->hAacDec, AAC_PCM_MAX_OUTPUT_CHANNELS, attr->info.channel_cnt);
    ...
    aac->pcm_frame_size = (attr->info.channel_cnt * attr->info.clock_rate * attr->info.pcm_bits_per_sample * attr->info.frm_ptime) / 8000;
